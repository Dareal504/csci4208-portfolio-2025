...
function initControls(){
    ...
    ups10s.addEventsListener("click", () => incrementEvent('tens'));
    up1s.addEventListener("click", ()=> incrementEvent('ones')); 

    const down100s = document.getElementById("up-100s");
    const down10s = document.getElementById("up-10s");
    const down1s = document.getElementById("up-1s");
    down100s.addEventListener("click", () => decrementEvent('hundreds'));
    down10s.addEventsListener("click", () => decrementEvent('tens'));
    down1s.addEventListener("click", ()=> decrementEvent('ones')); 

}

function buttonEvent(){
    const hunderds = document.getElementById("digit-100s");
